...
export class ObjectTypeDefinitionUtil {
	 ...
	 *
	 * @param typeDefinition The ObjectTypeDefinition for which to generate a
	 * JSON representation.
	 * @return A JSON representation of the provided
	 * {@link ObjectTypeDefinition}.
	 */
	public static typeDefinitionToReadableJSON(
		typeDefinition: ObjectTypeDefinition): any {
		...
		for (let element of iterator) {
			...
			if (element === undefined) {
				...
				throw new Error(
					"ERR | Attempted to use an undefined type definition."
				);
			...
			if (element.value.getTypeName !== undefined) {
				...
				result[element.key] = (element.value as Type).getTypeName();
				
			} else {
				
				result[element.key] =
					ObjectTypeDefinitionUtil.typeDefinitionToReadableJSON(
						element.value as ObjectTypeDefinition
					);
	 ...
	 * @return A string representation of the provided
	 * {@link ObjectTypeDefinition}.
	 */
	public static typeDefinitionToString(
		typeDefinition: ObjectTypeDefinition): string {
		
		function structureToStringArray(
			struct: ObjectTypeDefinition): string[] {
			
			let resultLines: string[] = [];
			let iterator: ObjectIterator<Type | ObjectTypeDefinition> =
				new ObjectIterator<Type | ObjectTypeDefinition>(struct);
			...
			for (let element of iterator) {
				...
				if (element.value instanceof AbstractType) {
					
					let typeName: string = element.value.getTypeName();
					...
					let nestedResultLines: string[] = structureToStringArray(
						element.value as ObjectTypeDefinition
					);
